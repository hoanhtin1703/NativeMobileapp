...
const ViewProductScreen = ({ navigation }) => {
  ...
  const handleDelete = (id) => {
    setIsloading(true);
    console.log(`${network.serverip}/delete-product/${id}`);
    fetch(`${network.serverip}/delete-product/${id}`, requestOptions)
        ...
        {foundItems && foundItems.length == 0 ? (
          ...
          foundItems.map((product, index) => {
            return (
              <ProductList
                key={index}
                image={`${network.serverip}/uploads/product/${product?.image_url}`}
                name={product?.name}
                category={
                  product.category_id
