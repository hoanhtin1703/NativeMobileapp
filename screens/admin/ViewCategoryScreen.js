...
const ViewCategoryScreen = ({ navigation }) => {
  ...
  const handleDelete = (id) => {
    ...
    setIsloading(true);
    fetch(`${network.serverip}/delete-category/${id}`, requestOptions) // API call
      ...
      >
        {foundItems && foundItems.length == 0 ? (
          ...
          foundItems.map((item, index) => (
            <CategoryList
              icon={`${network.serverip}/uploads/category/${item?.image_url}`}
              key={index}
              name={item.name}
