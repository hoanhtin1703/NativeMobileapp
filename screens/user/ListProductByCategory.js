...
const ListProductByCategory = ({ navigation, route }) => {
  ...
  const fetchProduct = () => {
    ...
    fetch(
      `${network.serverip}/getproductsbycategoryid/${categoryID}`,
      headerOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyContainer}>
        ...
        {/* {foundItems.filter((categories) => categories?.id === selectedTab?.id)
            .length === 0 ? (
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  <View style={styles.listProductCardContainer} key={item._id}>
                    <ProductCard
                      name={item.name}
                      image={`${network.serverip}/uploads/product/${item.image_url}`}
                      price={item.price}
